//
//  CustomARViewET.swift
//  AR Guide Service
//
//  Created by Suyeon Cho on 04/04/24.
//
...
struct CustomARViewETContainer: UIViewRepresentable {
    
    @Binding var eyeGazeActive: Bool
    @Binding var lookAtPoint: CGPoint?
    @Binding var isWinking: Bool
    
    func makeUIView(context: Context) -> CustomARViewET {
        return CustomARViewET(eyeGazeActive: $eyeGazeActive, lookAtPoint: $lookAtPoint, isWinking: $isWinking)
    }
    
    func updateUIView(_ uiView: CustomARViewET, context: Context) {}
}


class CustomARViewET: ARView, ARSessionDelegate {
